...
async function avanzar() {
	...
	let validaduyc = true;
	validaduyc == true ? (location.href = "usuario_table.html") : mostrarPopup2();

	const admins = await response.json();
	


	for (let admin_1 of admins.data) {
		alert(admin_1)
		alert(validaduyc)

		if (
			admin_1.usuario == document.getElementById("mail").value &&
			admin_1.contraseña == document.getElementById("contraseña").value) 
		{
			validaduyc = true;
		}
	}
